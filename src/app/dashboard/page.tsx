  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {['QC', 'LEAD', 'PM'].includes(currentUser?.role || '') && (
            <Link
              href="/defects/create"
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 text-xs font-bold text-white shadow-md shadow-blue-500/25 transition flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {/* New Open Today */}
        <div className="glass-panel p-5 border-t-4 border-t-blue-500">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium">Defect dilaporkan hari ini</div>
        </div>
        ...
        <div className="glass-panel p-5 border-t-4 border-t-purple-500">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium">Menunggu verifikasi QC</div>
        </div>

        {/* Re-opened Today */}
        <div className="glass-panel p-5 border-t-4 border-t-red-500">
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-bold mb-2">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium">Gagal verifikasi / regresi</div>
        </div>
        ...
        <div className="glass-panel p-5 border-t-4 border-t-emerald-500">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium">Selesai diperbaiki &amp; lolos uji</div>
        </div>

        {/* Total Outstanding */}
        <div className="glass-panel p-5 border-t-4 border-t-amber-500">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium">Akumulasi tiket aktif</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-panel p-6">
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-white/10">
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Distribusi Status Tiket</h3>
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Semua Bank</span>
          </div>
        ...
        <div className="glass-panel p-6">
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-white/10">
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Tingkat Keparahan (Active Defects)</h3>
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Severity Breakdown</span>
          </div>
          <div className="h-64 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-6">
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-white/10">
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-white flex items-center gap-2">
              <Building2 className="w-4 h-4 text-blue-600 dark:text-blue-400" /> Kualitas Defect per Bank Klien
            </h3>
            <Link href="/defects" className="text-xs text-blue-600 dark:text-blue-400 hover:underline font-bold flex items-center gap-1">
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {clientStats.map((cs: any) => (
                  <tr key={cs.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition">
                    <td className="py-3">
                      <div className="font-bold text-slate-900 dark:text-white">{cs.client_name}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{cs.client_code}</div>
                    </td>
                    <td className="py-3 text-center font-bold text-blue-600 dark:text-blue-400">{cs.count_open}</td>
                    <td className="py-3 text-center font-bold text-purple-600 dark:text-purple-400">{cs.count_retesting}</td>
                    <td className="py-3 text-center font-bold text-red-600 dark:text-red-400">{cs.count_reopen}</td>
                    <td className="py-3 text-center font-bold text-emerald-600 dark:text-emerald-400">{cs.count_close}</td>
                    <td className="py-3 text-center font-black text-slate-900 dark:text-white">{cs.total_defects}</td>
                    <td className="py-3 text-right">
                      <Link
                        href={`/defects?client_id=${cs.id}`}
                        className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-[11px] font-bold text-slate-700 dark:text-slate-300 transition"
                      >
                        Filter
                      </Link>
                    </td>
        ...
        <div className="glass-panel p-6">
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-white/10">
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-white flex items-center gap-2">
              <Activity className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Audit Aktivitas
            </h3>
          ...
          <div className="space-y-3.5">
            {recentActivities.map((act: any) => (
              <div key={act.id} className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-xs">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className="font-mono font-bold text-blue-600 dark:text-blue-400 text-[11px]">{act.ticket_number}</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400">{new Date(act.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <div className="text-slate-700 dark:text-slate-300 font-medium">
                  <strong className="text-slate-900 dark:text-white font-bold">{act.user_name}</strong> &rarr; <span className="font-bold text-purple-600 dark:text-purple-300">{act.to_status}</span>
                </div>
                {act.notes && (

      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white light:text-slate-900 tracking-tight flex items-center gap-2.5">
            <Bug className="w-6 h-6 text-blue-400" /> Daftar Defect Multi-Bank
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Kanban className="w-4 h-4 text-purple-400" /> Mode Kanban
          </Link>
        ...
        <form method="GET" action="/defects" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="lg:col-span-2 relative">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-white/10 rounded-xl py-2 pl-9 pr-3 text-xs text-white outline-none focus:border-blue-500"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full bg-slate-950/80 border border-white/10 rounded-xl py-2 px-3 text-xs text-white outline-none focus:border-blue-500"
            >
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 bg-slate-950/80 border border-white/10 rounded-xl py-2 px-3 text-xs text-white outline-none focus:border-blue-500"
            >
              ...
              {defects.length === 0 ? (
                ...
                defects.map((d: any) => (
                  <tr key={d.id} className="hover:bg-white/5 transition">
                    <td className="py-3">
                      <div className="font-mono font-bold text-blue-400">{d.ticket_number}</div>
                      <div className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                        <Building2 className="w-3 h-3 text-slate-500" /> {d.client_name}
                      </div>
                    </td>
                    <td className="py-3 max-w-xs">
                      <Link href={`/defects/${d.id}`} className="font-bold text-white light:text-slate-900 hover:text-blue-400 transition line-clamp-1">
                        {d.title}
                      </Link>
                      <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5 truncate">
                        <Layers className="w-3 h-3 text-purple-400" /> {d.project_name} &rsaquo; {d.module_name}
                      </div>
                    </td>
                    <td className="py-3">
                    ...
                    <td className="py-3 text-slate-300">
                      {d.dev_name || <span className="text-slate-500">Belum ditugaskan</span>}
                    </td>
                    ...
                    <td className="py-3 text-right">
                      <Link
                        href={`/defects/${d.id}`}
                        className="px-3 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 text-[11px] font-bold inline-flex items-center gap-1 transition"
                      >
